...
module.exports = function (RED) {
    ...
    var IRCServerNode = function (_RED$Node) {
        _inherits(IRCServerNode, _RED$Node);

        function IRCServerNode(n) {
            _classCallCheck(this, IRCServerNode);

            var _this = _possibleConstructorReturn(this, (IRCServerNode.__proto__ || Object.getPrototypeOf(IRCServerNode)).call(this, n));
            // RED.nodes.createNode(this, n);


            _this.server = n.server;
            _this.port = n.port || 6667;
            _this.ssl = n.ssl || false;
            _this.cert = n.cert || false;
            _this.channel = n.channel;
            _this.nickname = n.nickname;
            _this.lastseen = 0;
            _this.ircclient = null;
            _this.on("close", function () {
                try {
                    ...
                    console.log('error closing IRCServerNode', exp);
                }
            });

            _this.username = null;
            _this.password = null;
            if (_this.credentials && _this.credentials.hasOwnProperty("username")) {
                _this.username = _this.credentials.username;
            }
            if (_this.credentials && _this.credentials.hasOwnProperty("password")) {
                _this.password = _this.credentials.password;
            }
            return _this;
        }

        return IRCServerNode;
    }(RED.Node);
    ...
    var IrcInNode = function (_RED$Node2) {
        _inherits(IrcInNode, _RED$Node2);

        function IrcInNode(n) {
            _classCallCheck(this, IrcInNode);

            var _this2 = _possibleConstructorReturn(this, (IrcInNode.__proto__ || Object.getPrototypeOf(IrcInNode)).call(this, n));

            _this2.ircserver = n.ircserver;
            _this2.serverConfig = RED.nodes.getNode(_this2.ircserver);
            ...
            if (node.serverConfig.ircclient === null) {
                ...
                if (node.serverConfig.username !== null) {
                    options["userName"] = node.serverConfig.username;
                }
                if (node.serverConfig.password !== null) {
                    options["password"] = node.serverConfig.password;
                }
                ...
                node.serverConfig.ircclient.addListener('error', function (message) {
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.err") + ": " + JSON.stringify(message));
                    }
                });
                node.serverConfig.ircclient.addListener('netError', function (message) {
                    node.serverConfig.lastseen = Date.now();
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.net") + ": " + JSON.stringify(message));
                    }
                ...
                node.serverConfig.ircclient.addListener('connect', function () {
                    node.serverConfig.lastseen = Date.now();
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.connected"));
                    }
                });
                ...
                node.serverConfig.ircclient.addListener('ping', function (server) {
                    node.serverConfig.lastseen = Date.now();
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.ping") + " " + JSON.stringify(server));
                    }
                ...
                node.serverConfig.ircclient.addListener('quit', function (nick, reason, channels, message) {
                    node.serverConfig.lastseen = Date.now();
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.quit") + ": " + nick + " " + reason + " " + channels + " " + JSON.stringify(message));
                    }
                ...
                node.serverConfig.ircclient.addListener('raw', function (message) {
                    // any message received means we are alive
            ...
            node.ircclient.addListener('raw', function (message) {
                // any message means we are alive
                node.serverConfig.lastseen = Date.now();
            });
            node.on("close", function () {
                try {
                    ...
                    if (node.recon) {
                        clearInterval(node.recon);
                    }
                } catch (exp) {
                    console.log('error closing IrcInNode', exp);
                }
            });
            return _this2;
        }
    ...
    var IrcOutNode = function (_RED$Node3) {
        _inherits(IrcOutNode, _RED$Node3);

        function IrcOutNode(n) {
            _classCallCheck(this, IrcOutNode);

            var _this3 = _possibleConstructorReturn(this, (IrcOutNode.__proto__ || Object.getPrototypeOf(IrcOutNode)).call(this, n));

            _this3.sendFlag = n.sendObject;
            _this3.ircserver = n.ircserver;
            _this3.serverConfig = RED.nodes.getNode(_this3.ircserver);
            ...
            node.on("input", function (msg) {
                if (Object.prototype.toString.call(msg.raw) === '[object Array]') {
                    if (RED.settings.verbose) {
                        node.log(RED._("irc.errors.rawcommand") + ":" + msg.raw);
                    }
                    ...
                    if (node.sendFlag == "true") {
                        // override channels with msg.topic
                        if (msg.hasOwnProperty('topic') && typeof msg.topic === "string") {
                            ch = msg.topic.split(","); // split on , so we can send to multiple
                        } else {
                            node.warn(RED._("irc.errors.topicnotset"));
                        }
                    }
                    for (var c = 0; c < ch.length; c++) {
                        if (node.sendFlag == "false") {
                            // send whole message object to each channel
            ...
            node.on("close", function () {
                try {
                    ...
                    console.log('error closing IrcOutNode', exp);
                }
            });
            return _this3;
        }
