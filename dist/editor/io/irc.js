...
module.exports = function (RED) {
    ...
    RED.nodes.registerType('irc in', {
        ...
        renderDescription: function renderDescription() {
            return React.createElement('p', null, 'IRC input node.');
        }
    });
    ...
    RED.nodes.registerType('irc out', {
        ...
        render: function render() {
            return React.createElement('div', null, React.createElement(TextRow, { name: 'ircserver', icon: 'globe' }), React.createElement(TextRow, { name: 'channel', icon: 'random', placeholder: '#pagenodes' }), React.createElement(SelectRow, { name: 'sendObject', icon: 'arrows', options: [['send payload to channel(s)', 'pay'], ['Use msg.topic to set nickname or channel(s)', 'true'], ['Send complete msg object to channel(s)', 'false']] }), React.createElement(NameRow, null), React.createElement('div', { className: 'form-tips' }, React.createElement('span', null, 'The channel to join must start with a # Sending the complete object will stringify the whole msg object before sending.')));
        },
        renderHelp: function renderHelp() {
            return React.createElement('div', null, 'Sends messages to a channel on an IRC server');
        },
        renderDescription: function renderDescription() {
            return React.createElement('p', null, 'IRC output node.');
        }
    });
    ...
    RED.nodes.registerType('irc-server', {
        ...
        render: function render() {
            return React.createElement('div', null, React.createElement('div', { className: 'form-row' }, React.createElement('label', { htmlFor: 'node-config-input-server' }, React.createElement('i', { className: 'fa fa-globe' }), ' ', React.createElement('span', { 'data-i18n': 'irc.label.ircserver' })), React.createElement('input', { type: 'text', id: 'node-config-input-server', placeholder: 'irc.freenode.net', style: { width: '45%' } }), React.createElement('label', { htmlFor: 'node-config-input-port', style: { marginLeft: '10px', width: '35px' } }, ' ', React.createElement('span', { 'data-i18n': 'irc.label.port' })), React.createElement('input', { type: 'text', id: 'node-config-input-port', style: { width: '45px' } })), React.createElement('div', { className: 'form-row' }, React.createElement('label', null, '\xA0'), React.createElement('input', { type: 'checkbox', id: 'node-config-input-ssl', style: { display: 'inline-block', width: 'auto', verticalAlign: 'top' } }), React.createElement('label', { htmlFor: 'node-config-input-ssl', style: { width: '70%' } }, React.createElement('span', null, 'Use Secure SSL connection?'))), React.createElement('div', { className: 'form-row', id: 'certrow' }, React.createElement('label', null, '\xA0'), React.createElement('input', { type: 'checkbox', id: 'node-config-input-cert', style: { display: 'inline-block', width: 'auto', verticalAlign: 'top' } }), React.createElement('label', { htmlFor: 'node-config-input-cert', style: { width: '70%' } }, React.createElement('span', null, 'Allow self-signed certificates?'))), React.createElement(TextRow, { name: 'nickname', icon: 'heart', config: true }), React.createElement(TextRow, { name: 'username', icon: 'user', config: true }), React.createElement(TextRow, { name: 'password', icon: 'lock', config: true }));
        },
        renderHelp: function renderHelp() {
            return React.createElement('div', null, 'IRC server config');
        },
        renderDescription: function renderDescription() {
            return React.createElement('p', null, 'IRC server connection.');
        }
    });
};
